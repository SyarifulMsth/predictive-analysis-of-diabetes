...
Original file is located at
    https://colab.research.google.com/drive/1aBCGh9Xbm7Tv2WlZTct04x6XI3xCH3-9
...
# Your existing code for oversampling using SMOTE
oversample = SMOTE()
X_over, y_over = oversample.fit_resample(X, y)
counter_over = Counter(y_over)

# Visualization code for comparing before and after SMOTE
import matplotlib.pyplot as plt
...
# Before SMOTE
plt.subplot(1, 2, 1)
colors = ['#06D6A0', '#EF476F']  # Use green and red colors
...
random_forest__model = RandomForestClassifier(n_estimators=50, max_depth=5, max_features='auto')
...
print_score(random_forest_model_tuning, X_train, y_train, X_test, y_test, train=False)

"""## Best model

Based on the model training results, it can be seen that the best model is Random Forest Classification (Hyperparameter Tuning) with Train Accuracy  95.93% and Test Accuracy 84.09%. With the following classification report:
